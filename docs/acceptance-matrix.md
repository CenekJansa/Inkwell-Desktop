...
| 7.2 PDF display | M3.1-M3.5 | M3 private framing, sidecar sequencing/error mapping, desktop readiness gating, and Vue scrolling/navigation/zoom tests; PDFium packaging, real fixture rendering, and Windows 11 x64 evidence remain blocked by the distribution TODO |
...
| 9 Security and privacy | M2.1, M3.4, M8.1-M8.5 | M2 HMAC tests plus M3 bounded renderer frames, zeroized frame/document/image buffers, protocol-only sidecar stdout, fail-closed unconfigured backend, and Blob cleanup tests; runtime ACL, no-network, and no-temporary-file inspection follows on Windows |
| 10 Error handling | M1.1, M3.3, M4.5, M6.5, M8.4 | M3 renderer protocol and fake-engine tests cover invalid, encrypted, and render-failure categories; production PDFium classification and end-to-end fixtures remain pending |
| 11 User interface | M0.2, M3.5, M4.4, M6.1-M6.5 | M3 component tests cover explicit loading/error states, complete page stack, navigation, zoom, page failure, cancellation, Blob cleanup, and backend-authoritative progression gating; Windows usability checks remain pending |
